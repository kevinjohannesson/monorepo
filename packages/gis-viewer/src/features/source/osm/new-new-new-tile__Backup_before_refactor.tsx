...
// [] image caching mechanisme moet werken op osm tile parameters, nu werkt pannen niet meer, dit lost 't denk ik op.
...
function ImageTileLoader({
  ...
  useEffect(() => {
    ...
    //   ([x, y]) => !(osmTileImages[x]?.[y]?.[Math.floor(osmZoomLevel)] == null),
    // ).length;
    const totalFetchedTileImages2 = tileIndexOffsets.filter(
      ([x, y]) =>
        !(
    ...
    tileIndexOffsets.forEach(([x, y]) => {
      ...
      const avl1 = Object.keys(osmTileImages2).map(Number);

      const avl2 = [...avl1].reduce<number[]>((acc, z) => {
      ...
      if (isEmpty(filteredAvailableZoomLevels)) {
        if (totalFetchedTileImages2 !== totalValidTileNumbers) {
          const closestZoomLevel = getClosestNumber(avl2, osmZoomLevel);
